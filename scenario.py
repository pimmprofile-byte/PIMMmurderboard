...
# 공통지문 최종본 (v1.1)
COMMON_INTRO = (
    "우리는 모두 동창이다. 하지만 왜인지, 기억이 흐릿하다. 오래된 친구이지만 — 낯선 이들.\n"
    "어느 날, 의문의 쪽지가 도착했다. 「10년 만의 동창회. 졸업식 날 밤, 3학년 2반에서. — 동창회장」\n"
    "고등학생 때의 기억은 안개 같다. 떠올리려 하면, 손가락 사이로 빠져나간다.\n"
    "호기심이었을까. 추억에 대한 막연한 설렘이었을까. 우리는 같은 이유를 가지고, 같은 밤에, 같은 곳으로 향한다.\n"
    "동창회 장소는 인적 드문 폐교. 우리가 졸업한 학교다. 교문은 잠겨 있지 않았다.\n"
    "복도의 공기는 10년 전 겨울에서 멈춘 채 차갑고, 어디선가 스피커가 잡음을 삼키는 소리가 난다.\n"
    "낯이 익는다. 너무, 낯이 익는다.\n"
    "3학년 2반의 문을 연다. 달빛이 비추는 교실 한가운데 — 교복을 입은 누군가가 쓰러져 있다.\n"
    "처음 보는 시체. 그런데. 이 시체가, 내 동창이라고?"
)
...
CHARACTERS = [
    {
        ...
        "hidden": False,
        "sheet": [
            {"h": "자신에 대하여", "b": "늘 사람이 따르는 쪽이었다. 시키지 않아도 다들 내 말대로 움직였고, 나는 그걸 우정이라 불렀다. 지금은 작은 가게를 한다고 말하지만, 가게의 간판 색이 기억나지 않는다."},
            {"h": "기억의 균열", "b": "졸업식이 기억나지 않는다. 사진 한 장 없다. J와 어떻게 헤어졌는지도. 그 겨울 이후의 모든 것이 젖은 종이처럼 찢겨 있다."},
            {"h": "오승택과의 관계 (죄의 실체)", "b": "승택이는 소꿉친구다. 매점 빵, 과제, 가방 — 걔가 '해주던' 거다. 나는 시킨 적 없다. …정말 없나? 걔가 싫다고 한 적이 없었을 뿐인 건 아닌가. 이 질문을 나는 한 번도 스스로에게 한 적이 없다."},
            {"h": "그날의 진실", "b": "그 겨울 밤, 승택의 문자에 J의 사진이 붙어 있었다. '시키는 대로 해. 네가 나한테 그랬던 것처럼.' 나는 옥상으로 올라갔고 — 거부한다는 선택지가 없다는 게 어떤 것인지, 난간 앞에서 처음 알았다. 그리고 나는 떨어졌다."},
            {"h": "당신의 심경", "b": "두려운 건 라이터다. 내 라이터가 왜 시체 옆에 있나. 기억이 없는 밤에 내가 뭘 했을까. 그리고 그보다 깊은 공포 — 승택이 이야기가 나올 때마다, 내가 나쁜 놈이었을지도 모른다는 생각이 스치는 것. 나는 그 생각을 웃음으로 덮는다."},
            {"h": "자백의 순간 (트리거)", "b": "B4(심부름 쪽지 'SY') + C3(마지막 문자)가 모두 공론화되고, 누군가 '시키는 대로 한다는 게 어떤 기분인지 아느냐'고 물으면 방어가 무너진다. C4(추모석)까지 나오면 자기지목 확정."},
        ],
    ...
    },
    {
        ...
        "hidden": False,
        "sheet": [
            {"h": "자신에 대하여", "b": "손으로 만드는 것만이 나를 증명했다. 재능은 부족했고, 그래서 더 절박했다. 지금 무슨 일을 하는지 물으면 '작업을 한다'고 답하지만, 최근 작업이 하나도 떠오르지 않는다."},
            {"h": "기억의 균열", "b": "대학에 갔던가? 합격 발표를 본 기억이… 봤다고 믿고 있을 뿐, 장면이 없다. 그 겨울, 도자기를 감싸 안았던 감촉에서 기억이 끊긴다."},
            {"h": "오승택과의 관계 (죄의 실체)", "b": "말 한번 섞은 적 없다. 소문과, 그의 그림으로만 알았다. 그 그림이 문제였다 — 저 재능이 내 유일한 자리를 가져간다. 아무도 없는 미술실, 시너, 5분. 나는 사람을 미워한 게 아니라 그림을 지웠을 뿐이라고 되뇌었다. 그게 그의 유일한 동아줄인 걸 알면서."},
            {"h": "그날의 진실", "b": "그 겨울, 승택이 내 첫 입상작을 들고 나타났다. '네가 내 전부를 부쉈으니까.' 도자기가 도로 위에서 부서졌고, 나는 생각보다 먼저 몸이 움직였다. 파편을 줍던 손등 위로 헤드라이트가 쏟아졌다."},
            {"h": "당신의 심경", "b": "두려운 건 단추다. 미술실에 흘린 교표 단추 — 그게 나오면 끝이다. 나는 승택을 죽일 이유가 없다고 스스로 믿지만, '그의 인생을 죽인 것'에 대해서라면… 그 문장은 생각하지 않기로 한다."},
            {"h": "자백의 순간 (트리거)", "b": "D2(단추) + D3(실격 공문)이 공론화되고 누군가 단추의 주인을 직접 물으면 부분 인정('…그 날 미술실에 있었다'). D4(파편 상자)까지 나오면 전면 자백·자기지목."},
        ],
    ...
    },
    {
        ...
        "hidden": False,
        "sheet": [
            {"h": "자신에 대하여", "b": "나는 좋은 사람이다. 그렇게 말해오면 정말 그렇게 되는 줄 알았다. 봉사, 상담, 궂은일. 사람들이 고마워할 때의 그 표정을 나는 사랑했다 — 그 사람 자체보다도."},
            {"h": "기억의 균열", "b": "졸업 후 누구와도 연락한 기억이 없다. '내 사람들'이 그렇게 많았는데, 얼굴이 하나도 떠오르지 않는다. 그 겨울, 복도에 흩날리던 종이들에서 기억이 끊긴다."},
            {"h": "오승택과의 관계 (죄의 실체)", "b": "승택의 유일한 친구는 나였다. 곁에 있어줬고, 챙겨줬고 — 그런데 어느 날 걔가 물었다. '너 지금, 나 불쌍해서 좋지?' 나는 부정했지만, 그날 이후 걔를 피했다. 그리고 주변엔 말했다. '내가 걔한테 제일 잘해줬는데.' 뒤에서는 더한 말도 했다. 선의라는 이름의 가장 높은 자리에서, 나는 그를 내려다봤다."},
            {"h": "그날의 진실", "b": "그 겨울, 내 목소리가 담긴 유인물이 전교에 뿌려졌다. 한 장 한 장이 전부 사실이라서 반박할 수 없었다. 나를 보던 눈들 — 승택이 10년간 견딘 눈빛이 그거였다. 나는 그 시선 속에서 무너졌고, 스스로를 끝냈다."},
            {"h": "당신의 심경", "b": "두려운 건 녹취다. 손수건보다도. '좋은 사람'이 무너지는 것은 죽음보다 무섭다 — 실제로 그랬다. 그래서 나는 이 밤에도 중재자 자리를 먼저 차지한다. 그 자리에 있는 동안은 아무도 나를 심문하지 않으니까."},
            {"h": "자백의 순간 (트리거)", "b": "E3(녹취 유인물)이 공론화되면 '짜깁기'로 1차 방어. B5(장례의 조사)가 추가 공개되고 누군가 '조사를 읽으며 무슨 생각을 했느냐'고 물으면 무너진다. 자기지목 진입."},
        ],
...
MEMORY = {
    "sim": {"t1": "저 라이터… 내 거다. 왜 저기에?", "t2": "그 겨울, 승택이 옥상으로 불렀다. J의 사진과 함께. 그 다음이 없다.", "t3": "난간을 넘던 것은 — 나였다. 시키는 대로."},
    "yu":  {"t1": "저 조각칼… 내 거다. 잃어버린 줄 알았는데.", "t2": "그 겨울, 승택이 내 도자기를 들고 찾아왔다. 그 다음이 없다.", "t3": "도로 위 파편을 줍던 손 — 내 손이었다."},
    "lee": {"t1": "저 손수건… 내 거다. 승택에게 줬던 건데.", "t2": "그 겨울, 유인물이 뿌려졌다. 내 목소리였다. 그 다음이 없다.", "t3": "무너진 건 승택이 아니라 나였다. 그의 장례에서 조사를 읽은 것도, 나였다."},
