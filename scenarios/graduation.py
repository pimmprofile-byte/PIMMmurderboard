...
SCENE_NOTE = "3학년 2반 교실. 교복 차림의 오승택이 바닥에 누워 있다."
...
PHASES = [
    ...
    {"seq": 2, "key": "invest", "name": "1차 조사",       "min": 12, "ap": 2, "gm": "먼저, 이 시체가 누구인지부터. (각자 3장 조사)"},
    {"seq": 3, "key": "talk",   "name": "1차 토론",        "min": 10, "ap": 0, "gm": "물품 3점의 주인은 이 안에 있다. 오늘 밤 무슨 일이 있었나?"},
    {"seq": 4, "key": "invest", "name": "2차 조사",       "min": 12, "ap": 3, "gm": "학교가 기억하는 것을 찾아라. (각자 3장 조사)"},
    {"seq": 5, "key": "talk",   "name": "2차 토론",        "min": 10, "ap": 0, "gm": "오승택은 10년 전에 죽었다. 그렇다면 — 그 겨울, 당신들은 무엇을 했나?"},
    {"seq": 6, "key": "invest", "name": "3차 조사",       "min": 8,  "ap": 2, "gm": "마지막 문이 열렸다. (각자 2장 조사)"},
    {"seq": 7, "key": "talk",   "name": "마지막 토론",     "min": 10, "ap": 0, "gm": "죽인 사람이 꼭 손을 쓴 사람인가?"},
    {"seq": 8, "key": "final",  "name": "종막",  "min": 5,  "ap": 0, "gm": "각자 마지막 질문에 답한다. 오승택을 죽인 것은 누구인가 — 당신은 누구를 지목하는가."},
...
CHARACTERS = [
    ...
    {
        "id": "lee", "name": "이정민", "age": "32", "job": "봉사활동 이야기를 먼저 꺼냄", "avatar": "", "color": "#8ec98a",
...
def public_scenario() -> dict:
    return {
        ...
        "map": MAP, "victimCard": VICTIM_CARD, "sceneNote": SCENE_NOTE, "mapLabel": "학교 안",
...
- 진상 핵심('오세원의 정체', '우리가 이미 죽었다', '망령/진혼', 누가 왜 죽었는지의 전모)은 절대 먼저 말하지 마라. 너의 t3 기억이 떠오르기 전까지는 너 자신도 그걸 모른다.
- 확신이 없으면 밝히지 말고 얼버무려라. 침묵·"기억이 안 난다"·"글쎄"도 좋은 수다. 매 발언마다 정보를 주지 마라 — 열에 예닐곱은 감정·의심·변명뿐이어도 된다.
- 최종 토론(마지막 페이즈) 전에는 명시적 자백·자기지목 절대 금지.
...
_CARD_HINTS = {
    "A1": "손에 쥔 것부터 확인해야 한다.",
    "A2": "죽은 사람은 동창 오승택이다. 10년 전 무슨 일이 있었는지가 출발점.",
    "A3": "세 사람의 물건이 시신 옆에 있다. 왜 여기 있는지 각자 설명해야 한다.",
    "A4": "다섯 중 한 명의 얼굴만 오려졌다. 누가 빠졌는지 세어볼 것.",
    "A5": "오늘 죽은 게 아니다. 10년 전 그대로 여기 있었다.",
    "A6": "물건마다 주인이 있다. 이니셜과 출품명단을 맞춰볼 것.",
    "B1": "이 학교는 졸업식 그날에 멈춰 있다.",
    "B2": "그가 어떤 취급을 받았는지 남아 있다. 칼자국은 따로 봐야 한다.",
    "B3": "찢겨나간 마지막 장에 뭐가 적혀 있었는지 찾아야 한다.",
    "B4": "그는 미대를 준비했다. 심부름을 시킨 사람의 머리글자가 적혀 있다.",
    "B5": "장례에 온 사람이 한 명뿐이었다. 그 사람의 태도가 이상하다.",
    "C1": "옥상에서 무슨 사고가 있었다. 열쇠를 구해야 들어간다.",
    "C2": "누가 누구에게 사과하는 글인지, 머리글자로 좁혀볼 것.",
    "C3": "협박을 받고 시킨 대로 한 사람이 있다. 보낸 쪽과 받은 쪽을 가려야 한다.",
    "C4": "여기서 죽은 사람의 성이 보인다. 이름을 맞춰볼 것.",
    "D1": "두 사람이 같은 대회에 나갔다. 결과가 어떻게 됐는지 찾아볼 것.",
    "D5": "작품 훼손에 무엇이 쓰였는지 알려준다.",
    "D2": "누가 일부러 망가뜨렸다. 떨어진 단추가 실마리다.",
    "D3": "한 사람이 실격되고 다른 사람이 붙었다. 동기가 된다.",
    "D4": "유지호가 무엇을 죄책감으로 안고 사는지 보인다. 기사 속 사고가 누구 일인지 맞춰볼 것.",
    "E1": "오늘 밤 누가 이 방을 썼다. 초대장 보낸 사람과 이어질 수 있다.",
    "E2": "오승택이 미리 남긴 말이다. '너희가 한 일'이 뭔지가 남는다.",
    "E3": "이정민이 한 말이 퍼져서 그의 평판이 무너졌다. 누가 뿌렸는지 물어볼 것.",
    "E4": "초대장과 필체가 같다. 오늘 밤 우리를 부른 사람이 이 안에 있다.",
    "F1": "학교가 왜 문을 닫았는지, 사유가 뭉개져 있다.",
    "F2": "자살로 정리됐지만 맞지 않는 부분이 있다.",
    "F3": "그 겨울에 세 명이 더 죽었다. 이름을 다른 서류에서 찾아볼 것.",
    "F5": "옥상 열쇠다. 이걸 얻어야 옥상을 조사할 수 있다.",
    "F6": "명부에 없는 이름이 있다. 그 사람이 누구인지 따져봐야 한다.",
    "F4": "세 명이 졸업 전에 죽었다. 지금 여기 있는 사람들과 맞춰볼 것.",
}
