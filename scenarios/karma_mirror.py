...
업경은 죄를 비추는 거울이다. 어느 세계에서 죽은 사람은 다른 세계에서 사람을 죽인 자이고,
셋은 세 세계를 돌며 피해자 · 가해자 · 무고한 생존자를 한 번씩 맡는다. 다만 각자에게 배정된
세계는 자기가 무고한 세계뿐이라, 아무도 자기 죄를 못 본다. 남의 거울에만 비친다.

설계 정본: docs/업경_설계.md

⚠ 이 시나리오는 아직 플레이할 수 없다. 지금 서버는 CARDS 하나를 모두가 공유하는 구조라
...
# ── 세 세계 ──────────────────────────────────────────────────────
# 같은 셋이 세 세계에 존재한다. 세계마다 죽은 사람이 다르고, 죽인 사람도 다르다.
#
# 갑 → 을 → 병 → 갑 으로 한 바퀴 도는 고리다. 그래서 셋 다 정확히 한 번씩
# 피해자이고, 한 번씩 가해자이고, 한 번씩 무고한 생존자다.
#
#            세계 A        세계 B        세계 C
#   갑        피해자        가해자      무고한 생존자   ← 갑 플레이어는 여기
#   을     무고한 생존자     피해자        가해자        ← 을 플레이어는 세계 A
#   병        가해자      무고한 생존자    피해자        ← 병 플레이어는 세계 B
#
# 배정 규칙: 각자에게는 「자기가 무고한 세계」만 준다. 그래서 셋 다 진심으로
# 결백을 주장하고, 셋 다 옳고, 셋 다 남의 거울에서는 살인자다.
# 지목도 고리를 그린다 — 갑은 을을, 을은 병을, 병은 갑을 지목하게 된다.
WORLDS = [
    {"id": "A", "name": "첫 번째 거울", "victim": "gap",    "killer": "byeong",
     "alive": ["eul", "byeong"], "seat": "eul"},
    {"id": "B", "name": "두 번째 거울", "victim": "eul",    "killer": "gap",
     "alive": ["gap", "byeong"], "seat": "byeong"},
    {"id": "C", "name": "세 번째 거울", "victim": "byeong", "killer": "eul",
     "alive": ["gap", "eul"],    "seat": "gap"},
]
...
TIMELINE = []
TRUTH_FULL = (
    "업경은 죄를 비추는 거울이다. 세 세계는 같은 사건의 세 판본이 아니라, "
    "같은 셋이 자리를 바꿔 앉은 세 판본이다.\n\n"
    "갑 → 을 → 병 → 갑. 세계 A에서는 병이 갑을 죽였고, 세계 B에서는 갑이 을을 죽였고, "
    "세계 C에서는 을이 병을 죽였다. 그래서 어느 세계에서 애도받는 피해자는 "
    "다른 세계에서 사람을 죽인 자다. 죽음이 곧 그 세계에서 치른 값이다.\n\n"
    "플레이어에게는 각자 「자기가 무고한 세계」만 배정된다. 갑은 세계 C에, 을은 세계 A에, "
    "병은 세계 B에 앉는다. 그래서 셋 다 결백을 주장하는 것이 진심이고, 셋 다 그 주장이 옳다. "
    "지목도 고리를 그린다 — 갑은 을을, 을은 병을, 병은 갑을 지목하고, 셋 다 맞힌다.\n\n"
    "협력이 성립하는 이유도 여기 있다. 내 세계의 범인은 자기 흔적을 지웠지만, 그는 자기 세계에서 "
    "무고하므로 아무것도 숨기지 않는다. 그러니 그가 자기 방과 자기 습관을 결백하게 늘어놓는 순간, "
    "내 세계에서 그를 잡을 조각이 나온다. 셋은 서로를 도우면서 자기 목에 밧줄을 건다.\n\n"
    "마지막에 남는 것은 「누가 범인인가」가 아니라 「내 거울에는 무엇이 비쳤겠는가」다. "
    "각자는 끝까지 자기 죄를 보지 못한다. 남의 거울에만 비친다."
)
REDHERRINGS = []
...
# 범인은 세계마다 다르다(WORLDS[*]["killer"]). 엔진이 배역별 판정을 지원하기 전까지는
# 단일 값을 채울 수 없다 — 하나를 적어두면 그게 정답인 것처럼 채점된다.
CULPRIT_ID = ""
HIDDEN_ID = ""
...
FINAL_QUESTIONS = [
    ...
    "그 어긋남을 언제 눈치챘는가?",
    "당신이 죽은 세계가 있다면, 거기서 당신은 무엇을 하고 죽었겠는가?",
]
ENDINGS = {}
OPENING_CUTS = [
    {"img": "calm", "fx": "hush",
